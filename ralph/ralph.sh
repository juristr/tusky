...
echo -e "${DIM}Pushing branch and creating PR...${NC}"
...
if [ -z "$EXISTING_PR" ]; then
  # Read prd.json for PR details
  PROJECT=$(jq -r '.projectName // "project"' "$PRD_FILE" 2>/dev/null || echo "project")
  COMPLETED_STORIES=$(jq -r '[.userStories[] | select(.passes == true) | "- \(.id): \(.title)"] | join("\n")' "$PRD_FILE" 2>/dev/null || echo "- Stories implemented")

  gh pr create \
    --title "feat: [$PROJECT] - Ralph implementation" \
    --body "## Summary
Implemented via Ralph autonomous agent.

## Completed Stories
$COMPLETED_STORIES

---
🤖 Generated by Ralph" || true
fi
